...
\title{Ionospheric Disturbances produced by the 2019 Caribbean Meteoroid\tnoteref{tnote1}}%
...
%% Keywords
\KWD Space Sciences\sep Atmosphere \sep Ionosphere \sep UNAVCO%\sep Keyword3
\end{keyword}
\end{frontmatter}
...
%\begin{eqnarray}
%S'_{\mathrm{pg}} = \frac{S_{\mathrm{pg}} - \mathrm{min}(S_{\mathrm{pG}})}7%  {\mathrm{max}(S_{\mathrm{pG}} - \mathrm{min}(S_{\mathrm{pG}}))}
%\end{eqnarray}
...
%We thank Dr. Kalishin (AARI) and Dr. Sergeeva (SCiESMEX) for the opportunity to use Solar Terminator software.
